...
+ means the networking stack is up
+ ifs not guaranteed to still be configured
+ only use in After= for clean ahutdown
+ ifs are configured with routable IPs
+ use target only as last resort
+ services suppose to handle network reconfiguration gracefully
+ use with After= & Wants=
